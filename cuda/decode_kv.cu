...
__host__ void SSTDecode::DoDecode() {
  ...
  // printf("%llu %llu\n", footer.metaindex_handle_.offset_,
  // footer.metaindex_handle_.size_); printf("%u\n", DecodeFixed32(h_SST_ +
  // footer.metaindex_handle_.offset_ + footer.metaindex_handle_.size_ - 4));
  // printf("%u\n", ((uint32_t *)(h_SST_ +
  // footer.metaindex_handle_.offset_))[0]);
 ...
 * <<<grid, (4, y)>>>:
 * block中x维度必须为4，即4个线程共同协作生成一个DataBlock的CRC32
 */
