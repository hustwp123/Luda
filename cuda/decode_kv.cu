...
// Debug
__global__
void __test(const char *src, size_t cnt) {
	for (int i = 0; i < cnt; ++i) {
		if (src[i] == 0) {
			printf("i\n", i);
			assert(0);
...
void Debug::Test(const char *src, size_t cnt) {
	__test<<<1, 1>>>(src, cnt);
}
...
// 2. 中间部分按照4Bytes或者其它8Bytes等都行，进行处理
// 3. 尾巴部分也按照单字节单位，进行处理
__device__ 
void __copy_mm(char *dst, const char *src, size_t n) {
    int i, idx;
    int x = threadIdx.x;
...
__device__
void __MemcpyKernel(char *dst, const char *src, size_t n) {
    int i = 0;
    int x = threadIdx.x;
    int y = blockDim.x;
...
__global__ 
void MemcpyKernel(SST_kv *skv, int kv_cnt, char *base, char **SST) {
    int cur = 0;
...
/*
* 并行计算SST中 DATABLOCK对应的CRC32
* <<<grid, (4, y)>>>: block中x维度必须为4，即4个线程共同协作生成一个DataBlock的CRC32
*/
__global__
void GPUEncodeCRC32(char *SST_new, uint32_t *shared_offset, uint32_t *shared_size, int shared_cnt) {
...
__global__
void GPUEncodeCRC32_base(char *base, size_t n) {
   assert(blockDim.x == 4);
...
void GPUEncodeCopyShared(char **SST, char *SST_new, SST_kv *skv, int base_idx,
    ...
    int kv_cnt = kSharedKeys <= skv_cnt + base_idx - kv_start ? kSharedKeys : skv_cnt + base_idx - kv_start;
 ...
 */
__global__
void GPUEncodeFilter(char *SST_new, SST_kv *skv, filter_meta *fmeta, int f_cnt, int k, int __base) {
...
MGPU_HOST_DEVICE bool WpSlice::operator <(const WpSlice& b)
{
    ...
    {
        printf("get null1 data_==null:%d\n",data_==nullptr);
        return true;
    }
    else
    {
        printf("get null2 data_==null:%d\n",b.data_==nullptr);
        return false;
    }
...
void SSTSort::AllocLow(int size)
{
    ...
    {
        low_slices[i].data_=nullptr;
        low_slices[i].skv=nullptr;
    }
}
void SSTSort::AllocHigh(int size)
{
    ...
    for(int i=0;i<size;i++)
    {
        high_slices[i].data_=nullptr;
        high_slices[i].skv=nullptr;
    }
}
...
void SSTEncode::ComputeFilter() {
    ...
    //printf("%d %d\n", cur_ / 2048, offsets.size());
    //assert(cur_ / 2048 <= 2000);
...
void SSTEncode::WriteIndexAndFooter() {
    ...
    // meta_handler
    {
        ...
        {
            h_SST_[cur_] = 0;
            uint32_t result = leveldb::crc32c::Extend(0, h_SST_ + footer.metaindex_handle_.offset_,
    ...
    // index_block
    {
        ...
        {
            h_SST_[cur_] = 0;
            uint32_t result = leveldb::crc32c::Extend(0, h_SST_ + footer.index_handle_.offset_,
...
__host__ void SSTEncode::DoEncode() {
    ...
    GPUEncodeCopyShared<<<grid, block, 0, s1>>>(d_SST_ptr, d_SST_new_, d_skv_new_, base_, kv_count_, d_shared_offset_, shared_count_);
    //dim3 cblock(4, 8), cgrid(128, 1);
    //dim3 cblock(4, 8), cgrid(512, 1);
    dim3 cblock(4, 8), cgrid(M, 1);
    GPUEncodeCRC32<<<cgrid, cblock, 0, s1>>>(d_SST_new_, d_shared_offset_, d_shared_size_, shared_count_);
...

    {
        ...
        uint32_t result = leveldb::crc32c::Extend(0, h_SST_ + filter_handle_.offset_, filter_handle_.size_);
