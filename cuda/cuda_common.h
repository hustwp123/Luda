...
#define CUDA_MAX_COMPACTION_FILES (41)
...
struct GDI {        // Gpu Decode Info
  ...
  uint32_t
      kv_base_idx;  // SST_kv[]中一个GPU线程添加数据的起始槽下标，解码出来的多个共享前缀KV依次往后放
  uint32_t limit;
};

struct SST_kv {               // SST sorted KV pair
  char ikey[kKeyBufferSize];  // 解码出来的Key，[key + Seq + Type]
                              // GPU Encode后的 [shared + non_shared +
                              // value_size + partial_key ] 也放在这个地方
...
class SSTEncode {
 public:
  ...
  void ComputeDataBlockOffset(
      int sc = kDataSharedCnt);  // 每个DataBlock 默认有多少个共享前缀块
  ...
  filter_meta*
      h_fmeta_;  // 每个DataBlock对应的Filter元数据，包括多少个KV，从第几个开始等
  filter_meta*
      d_fmeta_;  // 每个DataBlock对应的Filter元数据，包括多少个KV，从第几个开始等
