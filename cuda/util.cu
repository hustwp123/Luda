...
__host__ __device__
void Memcpy(char *_dst, const char *_src, size_t size) {
    memcpy(_dst, _src, size);
    return ;

    uint8_t *p = (uint8_t *)_dst, *s = (uint8_t *)_src;
    uint8_t *e = (uint8_t *)_dst + size;

    uint8_t *p8 = reinterpret_cast<uint8_t*>(
          (reinterpret_cast<uintptr_t>(_dst) + 3) &
...
namespace gpu_crc32c {
    namespace {
        __device__ const uint32_t kByteExtensionTable[256] = {
        ...
        __device__ const uint32_t kStrideExtensionTable0[256] = {
        ...
        __device__ const uint32_t kStrideExtensionTable1[256] = {
        ...
        __device__ const uint32_t kStrideExtensionTable2[256] = {
        ...
        __device__ const uint32_t kStrideExtensionTable3[256] = {
        ...
        __device__
        inline uint32_t parallel_read(const uint32_t* buffer) {
        ...
        __device__
            inline const uint8_t* RoundUp(const uint8_t* pointer) {
                return reinterpret_cast<uint8_t*>(
                        (reinterpret_cast<uintptr_t>(pointer) + (4 - 1)) &
                        ~static_cast<uintptr_t>(4 - 1));
            }
    }  // namespace
    ...
    __device__ int PreRead() {
        for (int i = 0; i < 256; ++i) {
            uint32_t a = __ldg(&kByteExtensionTable[i]);
            uint32_t b = __ldg(&kStrideExtensionTable0[i]);
            uint32_t c = __ldg(&kStrideExtensionTable1[i]);
            uint32_t d = __ldg(&kStrideExtensionTable2[i]);
...
#define STEP4W(w)                                   \
        ...
        if ((e - p) >= 16) {
            ...
            */
            ...
            uint32_t crc, crc0, crc1, crc2, crc3;
            crc = parallel_read((uint32_t *)p);
            p += 16;
            if (threadIdx.x == 0) {
                crc = crc ^ l;
            }

            //printf("(%d %d) %u %u %u %u %u\n", threadIdx.x, threadIdx.y, crc, crc0, crc1, crc2, crc3);
            ...
            // Parallel
            while ((e - p) >= 16) {
                uint32_t tmp = parallel_read((uint32_t *)p);
                crc = tmp ^ __ldg(&kStrideExtensionTable3[crc & 0xff]) ^
                    __ldg(&kStrideExtensionTable2[(crc >> 8) & 0xff]) ^
                    __ldg(&kStrideExtensionTable1[(crc >> 16) & 0xff]) ^
                    __ldg(&kStrideExtensionTable0[crc >> 24]);

                p += 16;
            }
            // Exchange crc ==> crc1,2,3
            __shared__ uint32_t data[32];
            assert(blockDim.x * blockDim.y <= 32);
            data[threadIdx.y * blockDim.x + threadIdx.x] = crc;
            __syncthreads();
            crc0 = data[threadIdx.y * blockDim.x + 0];
            crc1 = data[threadIdx.y * blockDim.x + 1];
            crc2 = data[threadIdx.y * blockDim.x + 2];
            crc3 = data[threadIdx.y * blockDim.x + 3];
            //printf("(%d %d) %u %u %u %u\n", threadIdx.x, threadIdx.y, crc0, crc1, crc2, crc3);
