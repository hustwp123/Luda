...
// 在level层中获取所有有重叠的file到inputs这个vector中去
...
// GPU compaction是不需要关注这个地方的，不过也可以学习一下解码
//
// 这个地方比较复杂，因为牵涉到两层（如果是level0，那么每个SST都得单独用一个迭代器，否则是一层用一个迭代器）
// 所以需要将两层的迭代器整合到一起，但前提是需要先搞出每一个迭代器
  ...
  for (int which = 0; which < 2; which++) {
    if (!c->inputs_[which].empty()) {
      if (c->level() + which == 0) { // level 0 层
        ...
        // 创建一个层级迭代器，因为可能涉及多个SST，但是值并不会重复
...
Compaction* VersionSet::PickCompaction() {
  ...
  // Level0层直接把所有有Key重叠的都选中参与后面的Compaction
  if (level == 0) {
...
void VersionSet::SetupOtherInputs(Compaction* c) {
  ...
  // 看level层是否有其它可以合并的文件，如果总数目不多的情况下，直接拿来合并
  ...
  // key range next time.
  // 下次从Largest开始Compaction，也就是从小到大，如果没有更大，就到最小
  // 自然而然地就开始Round-Robin算法开始找
...
// 当一个生成的SST与level+2层重叠的bytes过多时候，就结束该SST
// 这样就可以避免下次合并这个生成的SST时候会牵涉特别多的SST
// 但是对于GPU compaction来说可以优化掉
bool Compaction::ShouldStopBefore(const Slice& internal_key) {
  const VersionSet* vset = input_version_->vset_;
  // Scan to find earliest grandparent file that contains key.
  const InternalKeyComparator* icmp = &vset->icmp_;

  // grandparents_ 是Level+2 层与需要合并的所有SST有重叠的SST
