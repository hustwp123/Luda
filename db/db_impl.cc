...
void thread_write_file_2(write_file* arg, int lo, int hi) {
  for (int i = lo; i < hi; i++) {
    write_file* pf = (write_file*)(arg + i);
    gpu::SSTEncode* pencode = pf->encode;
...
Status DBImpl::GPUWriteLevel0(const std::string& dbname, Env* env,
  ...
  if (iter->Valid()) {
    ...
    for (; iter->Valid(); iter->Next()) {
      ...
      pskv[kv_cnt].value_offset =
          (pskv[kv_cnt].value_offset & 0x00FFFFFF) | (sst_id << 24);
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  Status s;
  {
    ...
    s = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta);
#else
    s = GPUWriteLevel0(dbname_, env_, options_, table_cache_, iter, &meta);
#endif
    mutex_.Lock();
  }
...
void DBImpl::BackgroundCompaction() {
  ...
  // 构造c，实质是找inputs_[2]，也就是要合并的两层的所有SST
  if (is_manual) {
...
Status DBImpl::DoCompactionWork2(CompactionState* compact) {
  const uint64_t start_micros = env_->NowMicros();
  ...
  for (; input->Valid() && !shutting_down_.load(std::memory_order_acquire);) {
    ...
    if (compact->compaction->ShouldStopBefore(key) &&
        compact->builder != nullptr) {
      // 将创建的新的out SST，此时还是一个TableBuilder给下写到磁盘
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      // 有两种条件
      // 1. 已经有更新的（排序时候，seq越大排的越靠前），删除旧的，没有snap
      // 2. 标志是删除的，且没有snapshot
  ...
  stats_[compact->compaction->level() + 1].AddReason(
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  for (; input->Valid() && !shutting_down_.load(std::memory_order_acquire);) {
    ...
    // 首先把ImmTable给下写了
    if (1 && has_imm_.load(std::memory_order_relaxed)) {
      const uint64_t imm_start = env_->NowMicros();
      mutex_.Lock();
      if (imm_ != nullptr) {
        CompactMemTable();
 ...
 */
Status DBImpl::DoCompactionWork(CompactionState* compact, bool isSeek) {
  const uint64_t start_micros = env_->NowMicros();
  int64_t imm_micros = 0;  // Micros spent doing imm_ compactions
...
#define IMM_WRITE()
  ...
  if (!useWP) {
    ...
    for (auto& p : low_decode) {
      if (!p->inMem) p->DoGPUDecode_1();
    }
    for (auto& p : high_decode) {
      p->DoGPUDecode_1();
    }
    for (auto& p : low_decode) {
      if (!p->inMem) p->DoGPUDecode_2();
    }
    for (auto& p : high_decode) {
      p->DoGPUDecode_2();
    }
  ...
  // fprintf(stderr,"SST_kv_cnts=%d
  // last_keys==%d\n",SST_kv_cnts.size(),last_keys); for (int i = 0; i <
  // SST_cnt; ++i) {
  ...
  if (in.starts_with("num-files-at-level")) {
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
      ...
      if (stats_[level].micros > 0 || files > 0) {
        snprintf(
            buf, sizeof(buf), "%3d %8d %8.0f %9.0f %8.0f %9.0f %8ld  %8.1lf\n",
        ...
        sum_times_compact_reason_size +=
            stats_[level + 1].times_compact_reason_size;
        sum_times_compact_reason_seek +=
            stats_[level + 1].times_compact_reason_seek;
        sum_reason_size_bytes_read += stats_[level].reason_size_bytes_read;
        sum_reason_size_bytes_written +=
            stats_[level].reason_size_bytes_written;
        sum_reason_seek_bytes_read += stats_[level].reason_seek_bytes_read;
        sum_reason_seek_bytes_written +=
            stats_[level].reason_seek_bytes_written;
    ...
    snprintf(
        buf, sizeof(buf),
        "SUM                   %9.0f %8.0f %9.0f %8ld  %8.1lf\n",
        sum_micros / 1e6, sum_bytes_read / 1048576.0,
        sum_bytes_written /
            1048576.0,  // xp this includes flush imm_ to L0 sst, and
                        //   this = size compact + seek compact + sizeof(L0).
        sum_compact, sum_time_merge_cpu_micros / 1e6);
    value->append(buf);
