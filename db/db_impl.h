...
public:
  ...
  struct CompactionStats {
    ...
    void Add(const CompactionStats& c) {
      ...
      this->times_compact++; //xp
      this->time_merge_cpu_micros += c.time_merge_cpu_micros; // the us of input->Next() merging kv in DoCompactionWork()
    }
    ...
    void AddReason(const CompactionStats& s, CompactReason reason) {
      switch(reason) {
        case kSize:
          ...
          break;
        case kSeek:
          this->times_compact_reason_seek++;
    ...
    int64_t micros; //xp: covers the whole of DoCompactionWork()
    int64_t bytes_read;
    ...
    int64_t time_merge_cpu_micros; // the us of input->Next() merging kv in DoCompactionWork()
  ...
  CompactionStats stats_[config::kNumLevels+1] GUARDED_BY(mutex_);
